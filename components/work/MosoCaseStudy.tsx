...
/**
 * Moso case study body (everything below the hero) — shared by the standalone
 * page (`/work/moso`) and the expand overlay, which supplies its own hero.
 */
export function MosoCaseStudyBody() {
  return (
    <>
      {/* ── Stats strip ─────────────────────────────────────────────────────── */}
...
/**
 * Full Moso case study with its own illustration hero — used by the standalone
 * page `/work/moso` (direct navigation, no morph).
 */
export default function MosoCaseStudy() {
  return (
    <>
      <section
      ...
      >
        ...
        <motion.div
          ...
          transition={{ duration: 1.2, ease }}
        >
          <Image src="/img/work/moso_logo.svg" alt="Moso" width={120} height={24} className="opacity-60" unoptimized />
          ...
          <div className="flex flex-wrap gap-3">
            <a href="https://www.motionsociety.com" target="_blank" rel="noopener noreferrer" className={buttonVariants({ variant: "primary", size: "lg" })}>
              <span className="py-0.5 px-1">Visit motionsociety.com</span>
            </a>
          </div>
        </motion.div>
